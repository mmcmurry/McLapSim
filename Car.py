...
class Car:
    def __init__(self, name, mCar, powertrain, mu, a_brake, cla, cda, DAir):
        ...
        self.powertrain = powertrain# Average power of the engine, calculated using acceleration event times, in Watts
    ...
    def findStraightTime(self, v_i, v_f, length):
        ...
        a = 0
        v = v_i
        self.powertrain.selectGear(v_i)
        ...
        while(d < length - self.findBrakingDistance(v, v_f)):
            ...
            a = (max_force * throttle_percent) / self.mCar
            a_avg = (a + a_prev)/2
            v += a_avg*self.dt
            t += self.dt
            ...
            # print("Time: " + str(t))
            # print("RPM: " + str(self.powertrain.rpm) + "\tPower: " + str(self.powertrain.getPower()) + "\tTorque: " + str(self.powertrain.getTorque()) + "\tThrottle: " + str(throttle_percent))
